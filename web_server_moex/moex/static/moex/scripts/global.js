    ...
    function table_fill_with_iss_security_securities(table, secid) {
      if (!map_secid_jqXHR_iss_securities_security_json.has(secid)) {
        alert ("ERROR (" + table.table.attr("id") + "): no data received for security: " + secid);
        return false;
      }
      if (!map_secid_jqXHR_iss_securities_security_json.get(secid).has(map_key_security_securities_iss)) {
        alert ("ERROR (" + table.table.attr("id") + "): no securities data received for security: " + secid);
        return false;
      }
      var securities = map_secid_jqXHR_iss_securities_security_json.get(secid).get(map_key_security_securities_iss);
      if (securities === undefined || securities.data.length == 0) {
        console.log ("WARNING (" + table.table.attr("id") + "): 0 securities records available for security: " + secid);
        return false;
      }
      if (securities.data.length > 1) {
        console.log ("WARNING (" + table.table.attr("id") + "): Got more than 1 record for " + secid);
      }
      ...
      // translating here:
      data_columns.push({ title : securities.columns[0]                   }); // 0: "id"
      data_columns.push({ title : "Код ценной бумаги"                     }); // 1: "secid"
      data_columns.push({ title : "Краткое наименование"                  }); // 2: "shortname"
      ...
      data_columns.push({ title : securities.columns[6]                   }); // 6: "is_traded"
      ...
      $.each( securities.columns, function( i, val ) {
        if (val == "id")        { return; }
        if (val == "is_traded") { return; }

        var head = data_columns[i].title;
        var data = []
        
        switch (val) {
          case "type":
            var securitytype      = get_iss_index_securitytype_by_name  (securities.data[0][i]);
            if (securitytype) { 
              data.push({ data : securitytype.security_type_title, id : null });
            } else {
              data.push({ data : null, id : null });
            }
            break;
          case "group":
            var securitygroup     = get_iss_index_securitygroup_by_name (securities.data[0][i]);
            if (securitygroup) { 
              data.push({ data : securitygroup.title, id : null });
            } else {
              data.push({ data : null, id : null });
            }
            break;
          case "primary_boardid":
            var board_primary     = get_iss_index_board_by_boardid      (securities.data[0][i]);
            if (board_primary)      { 
              var badges = "";
              ...
              data.push({ data : board_primary.board_title + badges, id : null });
            } else {
              data.push({ data : null, id : null });
            }
            break;
          case "marketprice_boardid":
            var board_marketprice = get_iss_index_board_by_boardid(securities.data[0][i]);
            if (board_marketprice)  { 
              ...
              data.push({ data : board_marketprice.board_title + badges, id : null });
            } else {
              data.push({ data : null, id : null });
            }
            break;
          default:
            data.push({ data : securities.data[0][i], id : null });
            break;
        }

        var tr = table_tr(head, data, true)
        tr.addClass("d-flex");
        // https://stackoverflow.com/questions/25385289/how-to-set-the-size-of-a-column-in-a-bootstrap-responsive-table/25385798
        tr.find("th").addClass("col-6");
        tr.find("td").addClass("col-6");
        tr.appendTo(tbody);
      });
    ...
    function table_fill_with_iss_security_description(table, secid) {
      if (!map_secid_jqXHR_iss_securities_security_json.has(secid)) {
        alert ("ERROR (" + table.table.attr("id") + "): no data received for security: " + secid);
        return false;
      }
      if (!map_secid_jqXHR_iss_securities_security_json.get(secid).has(map_key_security_description)) {
        alert ("ERROR (" + table.table.attr("id") + "): no description data received for security: " + secid);
        return false;
      }
      var description = map_secid_jqXHR_iss_securities_security_json.get(secid).get(map_key_security_description);
      if (description === undefined || description.data.length == 0) {
        console.log ("WARNING (" + table.table.attr("id") + "): 0 description records available for security: " + secid);
        return false;
      }
      var tbody = $("<tbody/>")
      $.each( description.data, function( i, val ) {
        // 0: "name"
        // 1: "title"
        // 2: "value"
        // 3: "type"
        ...
        // 5: "is_hidden"
        // 6: "precision"
        var head = val[description.columns.indexOf("title")]
        var data = [
          { data : val[description.columns.indexOf("value")], id : null },
        ]
        var tr = table_tr(head, data, true);
        tr.addClass("d-flex");
        // https://stackoverflow.com/questions/25385289/how-to-set-the-size-of-a-column-in-a-bootstrap-responsive-table/25385798
    ...
    function table_fill_with_iss_security_boards(table, secid) {
      if (!map_secid_jqXHR_iss_securities_security_json.has(secid)) {
        alert ("ERROR (" + table.table.attr("id") + "): no data received for security: " + secid);
        return false;
      }
      if (!map_secid_jqXHR_iss_securities_security_json.get(secid).has(map_key_security_boards)) {
        alert ("ERROR (" + table.table.attr("id") + "): no boards data received for security: " + secid);
        return false;
      }
      var boards = map_secid_jqXHR_iss_securities_security_json.get(secid).get(map_key_security_boards);
      if (boards === undefined || boards.data.length == 0) {
        console.log ("WARNING (" + table.table.attr("id") + "): 0 boards records available for security: " + secid);
        return false;
      }
      ...
      var data_columns = new Array();
      // boards
      $.each(boards.columns, function (i, val) {
        data_columns.push({ title : val });
      })
      data_columns[boards.columns.indexOf("title")]         = ({ title : "Режим торгов" });
      data_columns[boards.columns.indexOf("history_from")]  = ({ title : "История с"    });
      data_columns[boards.columns.indexOf("history_till")]  = ({ title : "История по"   });
      data_columns[boards.columns.indexOf("listed_from")]   = ({ title : "Торги с"      });
      data_columns[boards.columns.indexOf("listed_till")]   = ({ title : "Торги по"     });
      data_columns[boards.columns.indexOf("currencyid")]    = ({ title : "Валюта"       });
      ...
      var tohide = [
        boards.columns.indexOf("secid"),
        boards.columns.indexOf("boardid"),
        boards.columns.indexOf("board_group_id"),
        boards.columns.indexOf("market_id"),
        boards.columns.indexOf("market"),
        boards.columns.indexOf("engine_id"),
        boards.columns.indexOf("engine"),
        boards.columns.indexOf("is_traded"),
        boards.columns.indexOf("decimals"),
        boards.columns.indexOf("is_primary"),
      ]
      ...
      function color (table) {
        $(table.table).find("tr").each( function (i, tr) {
          ...
          //console.log(arr_td[boards.columns.indexOf("title")]);
          if ($(arr_td[boards.columns.indexOf("title")]).find(".badge").length == 0) {
            // adding badges here...
            var badges = "";
            if ($(arr_td[boards.columns.indexOf("is_primary")]).text() == "1") {
              var badge = $("<span/>", {
                'class': "badge badge-success",  
                'html' : "основной"
              }).wrap('<p/>').parent().html();
            ...
            if ($(arr_td[boards.columns.indexOf("is_traded")]).text() == "0") {
              var badge = $("<span/>", {
                'class': "badge badge-danger",  
                'html' : "закрыт"
              }).wrap('<p/>').parent().html();
            ...
            var html = $(arr_td[boards.columns.indexOf("title")]).html();
            $(arr_td[boards.columns.indexOf("title")]).html(html + badges);
          }
        })
      }
      ...
      function google(table) {
        $(table.table).find("tr").each( function (i, tr) {
          ...
          // body
          var td = $(tr).find("td")[boards.columns.indexOf("title")];
...
//            'data-parent' : "#" + prefix_accordion + "main" 
    ...
    function grid_create_img_google (elem, after = false, force = false) {
      if (elem.has("img").length == 0) {

        if (elem.has(".search").length == 0 && force) {
          return;
        }

        // var string = force ? $(elem.find(".search")[0]).text() : elem.text();
        var string = "";
        if (force) {
          string = $(elem.find(".search")[0]).text();
        } else {
          // https://stackoverflow.com/questions/3442394/using-text-to-retrieve-only-text-not-nested-in-child-tags
          string = elem.clone()    //clone the element
        ...
        var img = $("<img/>",  {
          'src'    : NG_STATIC_FILES.SVG_QUESTION_CIRCLE,
          //'src'    : '{% static "moex/google.ico/favicon-16x16.png" %}', 
          'alt'    : "",
          'width'  : "16", 
          'height' : "16",
        });
        ...
        img.click( {param : [string]} , open);

        if (!after) {
          elem.prepend($("<div/>", { class : "divider" }));
          elem.prepend(img);
        } else {
          $("<div/>", { class : "divider" }).appendTo(elem);
          img.appendTo(elem);
        }
      }
      ...
      function open (event) {
        ...
        // this is to make sure there's no other events triggered (e.g. from clicking on table's td)
        event.stopPropagation();
        // console.log(event.data.param);
        var uri = "http://www.google.com/search?lr=lang_ru&q="; 
        $.each (event.data.param, function (i, string) {
          // https://stackoverflow.com/questions/6544564/url-encode-a-string-in-jquery-for-an-ajax-request
          // https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/encodeURIComponent
          uri += encodeURIComponent(" " + string).replace(/\s/, "+");
        })
        // console.log("google search: " + uri)
